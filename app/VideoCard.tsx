...
export default function VideoCard(props: {data: VideoData}) {
  ...
  return (
    <div className="VideoCard flex flex-col mx-auto p-5 rounded-lg border-4 justify-center text-center border-gray-800 bg-gray-400">
      <div>
        <Image src={data.thumbnail} width={640} height={480} alt="video thumbnail" />
      </div>
      ...
      <div className="mx-1">
        <button onClick={redirectToVideo} className="w-full py-2 border-2 border-black bg-red-500">
          <a href={`https://youtube.com/watch?v=${data.id}`} target="_blank">Watch</a>
        </button>
      </div>
    </div>
  );
